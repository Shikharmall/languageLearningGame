...
function DashboardCard05() {
  ...
  const [slicedLabels1, setSlicedLabels1] = useState(generateDates().slice(0, range).reverse());

  const [slicedData1, setSlicedData1] = useState(data.slice(0,35));

  const chartData = {
    //labels: slicedLabels,
    labels: slicedLabels1,
    datasets: [
      // Indigo line
      {
        //data: slicedData,
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-6 bg-white dark:bg-slate-800 shadow-md rounded-xl border border-slate-200 dark:border-slate-700">
      <header className="px-5 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center">
        <h2 className="font-semibold text-slate-800 dark:text-slate-100">Posts</h2>
        <Tooltip className="ml-2">
          <div className="text-xs text-center whitespace-nowrap"> Posts done on commune app </div>
        </Tooltip>
      </header>
